...
- Decide whether to keep or remove Emergent visual-edit development tooling in a separate frontend task.
- Decide whether to replace Emergent-hosted Google auth before production use; auth was not modified here.
...
OpenAI-only AI runtime settings:

```text
OPENAI_API_KEY=replace-with-openai-key
OPENAI_TEXT_MODEL=replace-with-approved-text-model
OPENAI_REALTIME_MODEL=gpt-realtime
OPENAI_VOICE=replace-with-approved-voice
```

`OPENAI_API_KEY` is optional for backend boot. If it is absent, AI-only routes return HTTP 503 instead of blocking FastAPI import. The planned full-duplex voice layer is OpenAI Realtime using `gpt-realtime`.

Optional values documented in `backend/.env.example` include CORS, public API URL, Perplexity, Twilio, Resend, facility defaults, device auth, and receipt signing settings.
...
- `MONGO_URL`, `DB_NAME`, and `JWT_SECRET` are required for import/startup in the current code.
- AI provider target is OpenAI only; no Claude/provider-router path is planned for this backend.
- `OPENAI_API_KEY` is required only when AI endpoints are used; missing keys produce HTTP 503.
- OpenAI Realtime / `gpt-realtime` is the planned full-duplex voice layer.
- Emergent Google auth remains in place by design for this task; auth was not modified.
