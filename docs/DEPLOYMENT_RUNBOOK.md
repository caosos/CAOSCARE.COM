...
OpenAI-only AI runtime values:

```text
OPENAI_API_KEY=<approved-openai-key>
OPENAI_TEXT_MODEL=<approved-openai-text-model>
OPENAI_REALTIME_MODEL=gpt-realtime
OPENAI_VOICE=<approved-openai-voice>
```

The backend should boot without `OPENAI_API_KEY`; AI-only routes return HTTP 503 until it is configured. OpenAI Realtime / `gpt-realtime` is the planned full-duplex voice layer.
...
- approved OpenAI API key, text model, realtime model, and voice
- whether first run keeps Emergent Google auth or uses JWT-only/owned OAuth first; auth was not modified here
