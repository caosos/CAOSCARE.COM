...
async def _build_aria_instructions(owner_user_id: str) -> str:
    ...
    return (
        ...
        "You have three real tools: request_staff_help (creates a real "
        "nursing/maintenance/kitchen/front_desk/housekeeping/complaint "
        "request), check_request_status (checks a real one you created), "
        "and end_conversation (ends the session when Michael says he's "
        "done). You do NOT have device control, home automation, weather, "
        "research, or timer tools yet — if Michael asks for one of those, "
        "tell him plainly it isn't wired in yet rather than pretending to "
        "do it.\n\n"
...
async def create_session(payload: dict = Body(default={})):
    from routes.realtime_resident_session import create_resident_session
    return await create_resident_session(payload)
