...
async def _run():
    ...
    try:
        ...
        view = _resident_safe_view(doc, "America/New_York")
        ...
        # CURRENT vs HISTORY (Level 1 "stop stale request resurrection" fix,
        # 3951ef6): a resolved request is no longer "current" - /status must
        # report nothing open rather than resurface stale lifecycle/spoken
        # text for a request that's already done. History stays queryable
        # via /resident-request/history on an explicit historical question.
        res = await resident_request_status(resident_id=rid)
        assert res["found"] is False, "a resolved request must not still be reported as current"
