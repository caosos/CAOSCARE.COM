...
export function useRealtimeVoice({
  ...
  const stop = useCallback((reason = "unspecified") => {
    if (pcRef.current || dcRef.current) {
      logSessionEnded(reason);
      logActivationClientEvent("session_ended_client", {
        room: ctxRef.current?.room, kiosk_id: ctxRef.current?.kiosk_id,
        alert_id: ctxRef.current?.alert_id, activation_id: ctxRef.current?.activation_id,
